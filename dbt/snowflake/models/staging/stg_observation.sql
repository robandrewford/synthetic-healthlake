...
renamed as (
    select
        ...
        record_content:category[0].coding[0].display::string as category_display,
        ...
        -- Encounter reference
        split_part(record_content:encounter.reference::string, '/', -1) as encounter_id,
        
        -- Effective date/time
        record_content:effectiveDateTime::timestamp as effective_datetime,
        record_content:effectivePeriod.start::timestamp as effective_period_start,
        ...
        record_content:valueCodeableConcept.coding[0].code::string as value_codeable_code,
        record_content:valueCodeableConcept.coding[0].display::string as value_codeable_display,
        
        -- Reference range
        record_content:referenceRange[0].low.value::float as reference_range_low,
        record_content:referenceRange[0].high.value::float as reference_range_high,
        record_content:referenceRange[0].low.unit::string as reference_range_unit,
        
        -- Interpretation
        record_content:interpretation[0].coding[0].code::string as interpretation_code,
        record_content:interpretation[0].coding[0].display::string as interpretation_display,
        
        -- Performer (First element)
        split_part(record_content:performer[0].reference::string, '/', -1) as performer_id,
