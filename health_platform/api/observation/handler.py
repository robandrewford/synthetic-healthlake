...
def search_observations(params: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        # Get total count
        count_results = execute_query(count_sql, tuple(query_params[:-2]) if query_params[:-2] else None)
...
def error_response(status_code: int, message: str) -> Dict[str, Any]:
    ...
    outcome = {
        ...
        'issue': [{
            'severity': 'error',
            'code': 'processing' if status_code >= 500 else 'not-found' if status_code == 404 else 'invalid',
            'diagnostics': message
        }]
    }
