    ...
    def generate_patient(self, patient_id: int) -> dict[str, Any]:
        ...
        if self.fake.boolean(chance_of_getting_true=10):
            patient["deceasedDateTime"] = (
                self.fake.date_time_between(start_date=birth_date, end_date="now").isoformat() + "Z"
            )
...
@click.option(
    "--bundle/--individual",
    default=False,
    help="Output as FHIR Bundle or individual files",
)
@click.option(
    "--format",
    type=click.Choice(["json", "ndjson"]),
    default="json",
    help="Output format (json=individual/bundle, ndjson=NDJSON)",
)
