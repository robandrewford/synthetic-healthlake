...
@click.option('--output-dir', '-o', required=True, type=click.Path(), help='Output directory for OMOP CSV files')
...
def main(count: int, output_dir: str, seed: int, format: str):
    ...
    if format == 'csv':
        ...
        click.echo(f"✓ Generated {count} persons with {len(all_conditions)} conditions and {len(all_measurements)} measurements")
        ...
        click.echo(f"✓ Generated {count} persons with {len(all_conditions)} conditions and {len(all_measurements)} measurements (Parquet)")
        click.echo(f"  Output: {output_path}")
