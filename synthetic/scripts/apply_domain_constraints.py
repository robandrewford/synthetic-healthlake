...
def validate_constraints(data_dir: Path, constraints: Dict[str, Any]) -> List[str]:
    ...
    for domain_name, domain_config in constraints.get('domains', {}).items():
        for constraint in domain_config.get('constraints', []):
            ...
            if field_name not in df.columns:
                violations.append(f"[{constraint_id}] Field '{field_name}' not found in {table_name}")
                continue
            ...
            if constraint_type == 'range':
                ...
                invalid_rows = df[
                    (df[field_name] < min_val) | (df[field_name] > max_val)
                ]
...
def main():
    parser = argparse.ArgumentParser(description="Validate synthetic data against domain constraints")
    ...
    parser.add_argument('--constraints-config', required=True, help='Path to domain constraints YAML')
    parser.add_argument('--distributions-config', required=True, help='Path to distribution profiles YAML')
    parser.add_argument('--terminology-dir', required=True, help='Directory with terminology mappings')
