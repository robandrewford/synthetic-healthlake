...
class TestOrgIsolationPatient:
    ...
    def mock_db_with_org_data(self):
        ...
        patient_data = {
            'org-a-test-001': [
                {'RECORD_CONTENT': {'id': 'pat-a-001', 'resourceType': 'Patient', 'name': [{'family': 'OrgA'}]}},
                {'RECORD_CONTENT': {'id': 'pat-a-002', 'resourceType': 'Patient', 'name': [{'family': 'OrgA'}]}},
            ],
            'org-b-test-002': [
                {'RECORD_CONTENT': {'id': 'pat-b-001', 'resourceType': 'Patient', 'name': [{'family': 'OrgB'}]}},
            ]
        }
...
class TestOrgIsolationIngestion:
    ...
    def test_webhook_stores_with_org_context(self):
        ...
        test_bundle = {
            ...
            'type': 'transaction',
            'entry': [
                {'resource': {'resourceType': 'Patient', 'id': 'test-pat'}}
            ]
        }
...
class TestOrgIsolationAudit:
    ...
    def test_access_logged_with_org_context(self):
        ...
        with patch('logging.Logger.info') as mock_log:
            ...
            mock_log("Processing request", extra={
